...
test.describe("Marla conversions", () => {
  ...
  test("converts Standard to Lahori Marla correctly", async ({ page }) => {
    ...
    await expect(page.getByText(/5.*Standard Marla.*=.*6\.0[45]\d.*Lahori Marla/i)).toBeVisible();
    await expect(page.getByText(/Equivalent area.*1[,]?361\.25.*sq ft/i)).toBeVisible();
  });

  test("converts Standard to Multani Marla correctly", async ({ page }) => {
    await page.getByLabel(/area.*marla/i).fill("5");
    await page.locator("#source-marla").selectOption("normal");
    ...
    await expect(page.getByText(/5.*Standard Marla.*=.*5\.04\d.*Multani Marla/i)).toBeVisible();
    await expect(page.getByText(/Equivalent area.*1[,]?361\.25.*sq ft/i)).toBeVisible();
  });

  test("converts Lahori to Standard Marla correctly", async ({ page }) => {
    ...
    await expect(page.getByText(/4.*Lahori Marla.*=.*3\.305[78].*Standard Marla/i)).toBeVisible();
  ...
  test("converts Multani to Standard Marla correctly", async ({ page }) => {
    await page.getByLabel(/area.*marla/i).fill("10");
    ...
    await expect(page.getByText(/10.*Multani Marla.*=.*9\.917[234].*Standard Marla/i)).toBeVisible();
